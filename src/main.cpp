...
#if defined(NDEBUG)
# error "Johnolivercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x19d430fba4b54cb88b85043580bc57444a52725951b4b2226db91f151977c041");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Johnolivercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Johnolivercoin Signed Message:\n";
...
{
    // Johnolivercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Johnolivercoin: 10k blocks
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Johnolivercoin: 1 days
static const int64 nTargetSpacing = 10 * 60; // Johnolivercoin: 10 minutes (block every 10 mins)
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Johnolivercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Johnolivercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    ...
    {
	//sum.blockchain
	//prevent getting johnoliver coin from joining other networks
        pchMessageStart[0] = 0xf1;
        pchMessageStart[1] = 0xc2;
        pchMessageStart[2] = 0xb6;
        pchMessageStart[3] = 0xd2;
	//sum.blockchain
	//genesis hash generated for testnet 
        hashGenesisBlock = uint256("0xd100d56c15bb21cd5d79eb6f0bf03ae7797da072f426a1cc6ceb72fbe6b32f2f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
	//sum.blockchain
	//To follow up on John Oliver's cryptocurrency episode, 
	//this project is to show how easy it is to rollout new currency
        const char* pszTimestamp = "John Oliver's Cryptocurrency episode hits 6+ million views in just 2 weeks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
	//sum.blockchain
	//coin value of the genesis block is same as for any block
	//(main.cpp around ln# 1090)
        txNew.vout[0].nValue = 1000 * COIN;
	//sum.blockchain
	//genesisblock hex key generated for this project
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043492044066fa64c5dd5e9ee77513cff79c1d5fe43d5063bc807886ce648220f467f5e609a172a655e7fb56d967e19e71379201cc0bb353a6a81645a8040173a9") << OP_CHECKSIG;
        CBlock block;
        ...
        block.nVersion = 1;
	//sum.blockchain
	//mainnet epic time (date +%s)
        block.nTime    = 1521937976;
        block.nBits    = 0x1e0ffff0;
	//sum.blockchain
	//generate nonce as part of genesisblock generation for mainnet
        block.nNonce   = 2085375121;

        if (fTestNet)
        {
	    //sum.blockchain
	    //testnet epic time (date +%s)
            block.nTime    = 1521937893;
	    //sum.blockchain
	    //generated for this project as part of genesis hash
            block.nNonce   = 386444123;
        }

//sum.blockchain
//code to generate genesis block -- START
//The literal boolean is set to true only the first time
//the literal boolean is switched back to false after the genesis block is generated
...
//sum.blockchain
//code to generate genesis block -- END
	...
	//sum.blockchain
	//generate new merkleroot for this project
        assert(block.hashMerkleRoot == uint256("0x5d1526d24214cf2cfb433357b9b117cf635e3450e18fb5ad826d8e88f5e5cfe6"));
        block.print();
...
//sum.blockchain
//prevent getting johnoliver coin from joining other networks for mainnet
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Johnolivercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JohnolivercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JohnolivercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JohnolivercoinMiner(CWallet *pwallet)
{
    printf("JohnolivercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("johnolivercoin-miner");
    ...
    try { loop {
        ...
        printf("Running JohnolivercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JohnolivercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JohnolivercoinMiner, pwallet));
}
